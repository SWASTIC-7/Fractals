...
const MAXIMUM_RAY_STEPS: i32 = 250;
const MAXIMUM_DISTANCE: f32 = 200.0;
...
fn mandelbulb(position: Vec3, time: f32) -> f32 {
    let mut z = position;
    let mut dr = 1.0;
    let mut r;
    
    // Animated power - oscillates between 8.0 and 13.0
    let power = 8.0 + 5.0 * map_range(f32::sin(time * PI / 10.0 + PI), -1.0, 1.0, 0.0, 1.0);
    ...
    for _ in 0..10 {
        ...
        dr = f32::powf(r, power - 1.0) * power * dr + 1.0;
        
        // Scale and rotate the point
        let zr = f32::powf(r, power);
...
fn distance_estimator(p: Vec3, time: f32) -> f32 {
    // Rotate for nice viewing angle
    let p = rotate_yz(p, -0.3 * PI);
    mandelbulb(p, time)
}
...
fn ray_march(ro: Vec3, rd: Vec3, time: f32) -> Vec4 {
    ...
    if hit {
        ...
        let hsv = vec3(0.8 + cur_pos.length() / 0.5, 1.0, 0.8);
        col = hsv_to_rgb(hsv);
    } else {
        // Glow effect for near-misses
        let hsv = vec3(0.8 + min_dist_to_scene_pos.length() / 0.5, 1.0, 0.8);
        let mut glow = hsv_to_rgb(hsv);
        glow = glow * (1.0 / (min_dist_to_scene * min_dist_to_scene).max(0.0001));
        let time_mod = map_range(f32::sin(time * 3.0), -1.0, 1.0, 3000.0, 50000.0);
        glow = glow / time_mod;
        col = glow;
    }
    
    // Apply ambient occlusion based on steps
    let mut final_col = col / (steps * 0.08).max(0.01);
...
pub fn main_fs(
    ...
    // Camera setup - looking at the origin from behind
    let ro = vec3(0.0, 0.0, -2.0);
    
    // Ray direction towards +Z (looking at origin)
    let rd = get_ray_direction(uv, ro, vec3(0.0, 0.0, 1.0), 1.0);
