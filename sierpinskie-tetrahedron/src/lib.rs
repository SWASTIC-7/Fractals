...
/// Sierpinski Tetrahedron SDF using space folding
fn sierpinski_tetrahedron(mut z: Vec3) -> f32 {
    let iterations = 15;
    let scale = 2.0;
    let offset = 3.0;
    ...
    let mut n = 0;
    while n < iterations {
        // Fold 1: if x + y < 0, swap and negate
        if z.x + z.y < 0.0 {
            let temp = z.x;
            z.x = -z.y;
            z.y = -temp;
        }
        // Fold 2: if x + z < 0, swap and negate
        if z.x + z.z < 0.0 {
            let temp = z.x;
            z.x = -z.z;
            z.z = -temp;
        }
        // Fold 3: if y + z < 0, swap and negate
        if z.y + z.z < 0.0 {
            let temp = z.y;
            z.y = -z.z;
            z.z = -temp;
        ...
        // Scale and offset
        z = z * scale - vec3(offset * (scale - 1.0), offset * (scale - 1.0), offset * (scale - 1.0));
        n += 1;
    }
    
    // Return distance estimate
    z.length() * f32::powf(scale, -(n as f32))
}

/// Distance estimator for the scene
fn distance_estimator(p: Vec3) -> f32 {
    // Rotate the point for nice viewing angle
    let p = rotate_yz(p, 0.20 * PI);
    let p = rotate_xz(p, 0.25 * PI);
    sierpinski_tetrahedron(p)
}
...
fn ray_march(ro: Vec3, rd: Vec3, time: f32) -> Vec4 {
    ...
    if hit {
        ...
        let hsv = vec3(0.8 + cur_pos.length() / 8.5, 1.0, 0.8);
        col = hsv_to_rgb(hsv);
    } else {
...
pub fn main_fs(
    ...
    // Vertical tilt (fixed angle since we don't have mouse)
    ro = rotate_yz(ro, -PI + 1.1);
    
    // Horizontal rotation based on time
